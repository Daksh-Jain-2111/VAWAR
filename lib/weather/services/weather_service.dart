    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < dates.length && i < 7; i++) {
          final date = dates[i];
          final maxTemp =
              (i < tempMax.length ? tempMax[i] : null)?.toDouble() ?? 0.0;
          final minTemp =
              (i < tempMin.length ? tempMin[i] : null)?.toDouble() ?? 0.0;
          final avgTemp = (maxTemp + minTemp) / 2.0;
          final rainfall =
              (i < precipitation.length ? precipitation[i] : null)
                  ?.toDouble() ??
              0.0;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < dates.length && i < 7; i++) {
          ...
          final avgTemp = (maxTemp + minTemp) / 2.0;
          final rainfall =
              (i < precipSum.length ? precipSum[i] : null)?.toDouble() ?? 0.0;
          final weatherCode =
              (i < weatherCodes.length ? weatherCodes[i] : null) ?? 0;
